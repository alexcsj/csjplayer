...
- 影片畫面右鍵選單:顯示媒體內容(解析度、編碼、影格率、位元率等詳細資訊)
...
`dist/csjplayer-*.AppImage` 是不需安裝、不需系統裝 Qt6/mpv 就能執行的可攜版本,適合拿去其他電腦用:
...
| 影片畫面按右鍵 | 開啟選單:顯示媒體內容(解析度、編碼、影格率、位元率等) |
...
進度條上會顯示已設定的循環點:綠色三角形是起點、紅色是終點。只設定其中一個點時,循環會立即生效(另一個點自動視為影片開頭/結尾),但因為那個點不是你特意設的,進度條上不會顯示對應的標記。切換播放清單裡的檔案時會自動清除循環設定。
...
開啟檔案/增加檔案/增加資料夾/匯出入清單用的檔案對話框,左側側邊欄預設有家目錄、桌面、文件、下載、影片、音樂、圖片等常用資料夾。可以把資料夾從右邊的檔案列表拖曳到左側側邊欄來加入捷徑,或右鍵移除;變更會記住,下次開啟時還在。
