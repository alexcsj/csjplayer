# csjplayer

以 FFmpeg(libmpv)為核心引擎的自製 Qt6 影音播放器。永久無邊框視窗、自製標題列,播放清單、A-B 循環、0.1x~32x 正逆向變速播放等功能。

## 功能特色

- 播放/暫停、可點擊與拖曳的進度條
- 播放清單:資料夾遞迴掃描、拖曳加入、增加/移除項目、匯出入 `.m3u` 清單檔
- 三態循環模式:不循環 / 清單循環 / 單集循環 / 播完暫停
- 音量調整、靜音、滑鼠滾輪/鍵盤快捷鍵控制
- 播放速度 0.1x ~ 32x(正向)、0.1x ~ 8x(逆向,原生倒播搭配自動 fallback 模擬倒播)
- A-B 循環片段
- 開檔對話框、拖曳檔案/資料夾到視窗即可播放
- 永久無邊框視窗,自製標題列(拖曳移動、邊緣縮放、最小化/最大化/關閉)
- Alt+1~5 視窗尺寸快捷鍵(含直向尺寸,皆可自訂)、Ctrl+/ 隱藏所有面板(乾淨播放模式)
- 全螢幕切換(左右鍵同按 / Ctrl+Enter)、Esc / Ctrl+Q 結束程式
- 影片畫面右鍵選單:顯示媒體內容(解析度、編碼、影格率、位元率等詳細資訊)、調整快轉/回轉時間、調整視窗大小快捷鍵、調整音訊/視訊同步

## 建置

### 需求

- CMake ≥ 3.21
- C++17 編譯器
- Qt6(Widgets、OpenGLWidgets)
- libmpv(開發套件,含 `mpv/client.h`、`mpv/render.h`)
- ffmpeg(執行期需要,`ffmpeg` 指令要在 PATH 中——「調整音訊/視訊同步」的「同時寫入檔案」功能會呼叫它)

Arch Linux 上安裝依賴:

```bash
sudo pacman -S cmake qt6-base mpv ffmpeg
```

### 編譯

```bash
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build -j$(nproc)
```

執行檔會在 `build/csjplayer`。直接執行,或帶一個影片路徑當參數:

```bash
./build/csjplayer /路徑/影片.mp4
```

## 安裝

### Arch Linux 套件(PKGBUILD)

```bash
makepkg -si
```

會自動處理相依套件、安裝執行檔、`.desktop` 檔案與圖示。之後可直接在應用程式選單啟動,或在終端機打 `csjplayer`。解除安裝:

```bash
sudo pacman -R csjplayer
```

### Portable AppImage(任何 Linux 發行版)

`dist/csjplayer-*.AppImage` 是不需安裝、不需系統裝 Qt6/mpv 就能執行的可攜版本,適合拿去其他電腦用:

```bash
chmod +x csjplayer-*.AppImage
./csjplayer-*.AppImage
```

AppImage 預設固定使用 XCB(X11)顯示後端執行,即使在 Wayland 桌面也是透過 XWayland 相容層運作正常;這是刻意的設計,因為打包時的 Qt6 Wayland 平台外掛在不同機器上容易有 ABI 不相容問題。

若要自己重新產生 AppImage,需要 [linuxdeploy](https://github.com/linuxdeploy/linuxdeploy)、[linuxdeploy-plugin-qt](https://github.com/linuxdeploy/linuxdeploy-plugin-qt)、[appimagetool](https://github.com/AppImage/appimagetool),流程大致是:Release 建置 → `DESTDIR=AppDir cmake --install build` → 用 linuxdeploy 的 qt 外掛蒐集依賴函式庫 → 用 appimagetool 封裝。

## 快捷鍵總覽

### 播放控制

| 按鍵 | 動作 |
|---|---|
| Space | 播放/暫停 |
| 雙擊影片畫面 | 播放/暫停 |
| ← / → | 回轉/快轉 10 秒(可調整) |
| Ctrl+← / Ctrl+→ | 回轉/快轉 5 分鐘(可調整) |
| 按住 Z + ← / → | 回轉/快轉 1 秒(可調整) |
| 按住 X + ← / → | 回轉/快轉 30 秒(可調整) |
| 按住 C + ← / → | 回轉/快轉 1 分鐘(可調整) |
| 滑鼠水平滾輪(傾斜) | 同左右鍵;Ctrl+ 則同 Ctrl+左右鍵 |
| ↑ / ↓ | 音量 +/− |
| 滑鼠垂直滾輪 | 音量 +/− |
| PageDown / PageUp | 播放清單下一首 / 上一首 |
| 影片畫面按右鍵 | 開啟選單:顯示媒體內容(解析度、編碼、影格率、位元率等)、調整快轉/回轉時間、調整視窗大小快捷鍵、調整音訊/視訊同步 |
| Esc / Ctrl+Q | 結束程式 |

右鍵選單的「調整快轉/回轉時間」可以個別修改上面五種快轉/回轉按鍵(無修飾鍵、Z、X、C、Ctrl)的秒數,設定會記住。

右鍵選單的「調整音訊/視訊同步」可以用毫秒(ms)為單位微調音訊/視訊的時間差:正數代表音訊比視訊慢(延遲音訊),負數代表音訊比視訊快(提前音訊)。此設定對目前播放的檔案立即生效(僅限本次播放,重開檔案會重置)。

勾選對話框中的「同時寫入檔案」則會另外呼叫 `ffmpeg` 以串流複製(無需重新編碼)的方式,把調整後的同步狀態永久寫入一份新檔案(檔名加上 `_synced` 後綴,與原始檔案同資料夾),不會覆蓋原始檔案;寫好的新檔案在任何播放器上都會維持調整後的同步狀態。此功能需要系統已安裝 `ffmpeg`。

### A-B 循環

| 按鍵 | 動作 |
|---|---|
| `[` | 設定循環起點(A 點) |
| `]` | 設定循環終點(B 點) |
| `\` | 取消循環 |

進度條上會顯示已設定的循環點:綠色三角形是起點、紅色是終點。只設定其中一個點時,循環會立即生效(另一個點自動視為影片開頭/結尾),但因為那個點不是你特意設的,進度條上不會顯示對應的標記。切換播放清單裡的檔案時會自動清除循環設定。

### 視窗

| 按鍵/手勢 | 動作 |
|---|---|
| Alt+1 / Alt+2 / Alt+3 | 視窗尺寸切換為 960×540 / 1920×1080 / 3840×2160(可調整) |
| Alt+4 / Alt+5 | 視窗尺寸切換為 540×960 / 1080×1920(直向,可調整) |
| Alt+0 | 視窗尺寸切換為目前播放影片的原始解析度 |
| Ctrl+/ | 切換隱藏/顯示標題列與所有控制面板 |
| 滑鼠左鍵拖曳(影片畫面或標題列) | 移動視窗 |
| 滑鼠左鍵拖曳(視窗上下左右邊緣) | 縮放視窗(單方向,不鎖比例) |
| 滑鼠左鍵拖曳(視窗四個角落) | 等比例縮放視窗 |
| 雙擊標題列 | 最大化/還原 |
| 滑鼠左右鍵同時按下 / Ctrl+Enter | 切換全螢幕 |

右鍵選單的「調整視窗大小快捷鍵」可以個別修改 Alt+1~5 各自對應的寬×高,以及邊緣縮放判定框的粗細(預設 5px),設定會記住。

⚠️ 角落等比例縮放有個小限制:不管拖哪一個角落,視窗實際上都是以左上角為錨點往右下方縮放(不會真的從你拖的那個角落長出去)。這是因為 Wayland 底下應用程式無法自己移動視窗位置,只能改變大小,所以沒辦法讓左上/右上/左下角落做到「從那個角落長大」的視覺效果,但等比例縮放本身是正常運作的。

### 播放清單面板

| 操作 | 動作 |
|---|---|
| + | 增加檔案(附加到清單) |
| 📁+ | 增加資料夾(遞迴掃描,附加到清單) |
| − / Delete / Backspace | 移除選取的項目 |
| Shift+點選 / Ctrl+點選 | 整排選取 / 多選加選 |
| 雙擊項目 | 播放該項目 |
| 匯出清單 / 匯入清單 | 存成或讀取 `.m3u` 播放清單檔 |

### 播放速度

底部控制列的方向鈕(▶/◀)切換正向/倒轉播放,下拉選單選擇倍速(0.1x~32x,倒轉時限制在 8x 以內)。倒轉播放優先使用 mpv 原生倒播,若該檔案/編碼不支援會自動切換為模擬倒播(此模式無聲音)。切換播放清單裡的檔案時,會自動重置回正向 1x。

### 檔案對話框側邊欄

開啟檔案/增加檔案/增加資料夾/匯出入清單用的檔案對話框,左側側邊欄預設有家目錄、桌面、文件、下載、影片、音樂、圖片等常用資料夾。可以把資料夾從右邊的檔案列表拖曳到左側側邊欄來加入捷徑,或右鍵移除;變更會記住,下次開啟時還在。

## 已知限制

- **四角吸附未實作**:Wayland 協定不允許應用程式自行設定視窗絕對位置(不論是拖曳中或程式化呼叫),此功能在 Wayland 桌面環境下無法實現,已放棄嘗試。
- **視窗永遠置頂未實作**:同樣是 Wayland 的限制,標準協定沒有讓應用程式要求「永遠在最上層」的機制。多數桌面環境(如 GNOME)有自己的視窗選單可以達成這個效果(嘗試按 Alt+Space 或 Super+Space 開啟視窗操作選單)。
- 模擬倒播模式下沒有聲音(mpv 沒有音訊倒放的機制)。
